...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    //the following code was removed once we created the class TWCoreDataHelper
    //    id delegate = [[UIApplication sharedApplication] delegate];
    //    NSManagedObjectContext *context = [delegate managedObjectContext];
    
    NSError *error = nil;
    
    // the following code changes to the code directly below it
    //    NSArray *fetchAlbums = [context executeFetchRequest: fetchRequest error:&error];
    
    NSArray *fetchAlbums = [[TWCoreDataHelper managedObjectContext] executeFetchRequest: fetchRequest error:&error];
...
{
    //    removed after adding TWCoreDataHelper
    //    id delegate = [[UIApplication sharedApplication] delegate];
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex == 1) {
        ...
        //the above three lines replaces the three lines below.  notice how the NSIndexPath * indexpath line is insereted into @[]
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([segue.identifier isEqualToString:@"Album Chosen"]){
        if ([segue.destinationViewController isKindOfClass:[TWPhotosCollectionViewController class]]) {NSIndexPath *path = [self.tableView indexPathForSelectedRow];
